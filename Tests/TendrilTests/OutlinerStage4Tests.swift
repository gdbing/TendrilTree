...
@Suite final class OutlinerStage4Tests {
    ...
    private func verifyIndentations(
        in tree: TendrilTree, expectedIndents: [Int], file: StaticString = #file, line: Int = #line
    ) throws {
        var currentOffset = 0
        var lineIndex = 0
        ...
        guard lines.count == expectedIndents.count else {
            Issue.record(
                "Mismatch between number of lines (\(lines.count)) and expected indentations (\(expectedIndents.count))"
            )
            return
        }
        ...
        for lineContent in lines {
            ...
            if currentOffset < tree.length {  // Only check valid offsets
                ...
                #expect(
                    depth == expectedIndents[lineIndex],
                    "Line \(lineIndex) expected indent \(expectedIndents[lineIndex]), got \(depth)")
